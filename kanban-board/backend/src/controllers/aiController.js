...
const generateTasks = asyncHandler(async (req, res) => {
  ...
  const colRes = await query(
    ...
    [req.body.column_id, req.body.board_id],
  );
  ...
  for (const s of suggestions) {
    ...
    const { rows } = await query(
      "INSERT INTO tasks (board_id, column_id, title, description, priority, position, created_by) VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING *",
      [
        req.body.board_id,
        req.body.column_id,
        s.title,
...
const breakdownTask = asyncHandler(async (req, res) => {
  ...
  if (req.body.taskId) {
    const { rows } = await query(
      ...
      [req.body.taskId, req.body.board_id],
    );
