...
const listBoard = asyncHandler(async (req, res) => {
  const { rows } = await query(
    `SELECT b.*, (b.owner_id = $1) as is_owner,
      (SELECT COUNT(*) FROM tasks t WHERE t.board_id = b.id) as task_count,
      (SELECT COUNT(*) FROM board_members m WHERE m.board_id = b.id) as member_count FROM boards b
...
const createBoard = asyncHandler(async (req, res) => {
  ...
  const board = await withTransaction(async (client) => {
    ...
    const b = rows[0];
    console.log("req.user - ", b, req.user);
    await client.query(
...
const getBoard = asyncHandler(async (req, res) => {
  ...
  const boardData = await Promise.allSettled([
    ...
    query(
      `SELECT t.*, a.name AS assignee_name, a.email AS assignee_email, a.avatar_url AS assignee_avatar 
        FROM tasks t 
        LEFT JOIN users a ON a.id = t.assignee_id WHERE t.board_id = $1 ORDER BY t.position ASC`,
      [boardId],
    ),
    query(
      `SELECT u.id, u.name, u.email, u.avatar_url, m.role, m.joined_at 
        ...
        WHERE m.board_id = $1 ORDER BY t.position ASC`,
      [boardId],
    ),
  ]);
  ...
  //   ),
  // ]);
  const [boardRes, columnsRes, tasksRes, membersRes] = boardData;

  res.json({
    board: boardRes.status === "fulfilled" ? boardRes.value.rows[0] : null,
    columns: columnsRes.status === "fulfilled" ? columnsRes.value.rows : null,
    tasks: tasksRes.status === "fulfilled" ? tasksRes.value.rows : null,
    members: membersRes.status === "fulfilled" ? membersRes.value.rows : null,
    role: req.board.role,
  });
});
...
const getActivity = asyncHandler(async (req, res) => {
  ...
  const { rows } = await query(
    `SELECT act.*, u.uname AS user_name, u.avatar_url AS user_avatar
     FROM activities act
